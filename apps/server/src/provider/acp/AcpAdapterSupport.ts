...
export function mapAcpToAdapterError(
  ...
  // Process exits keep their stderr in a process error so session-start
  // failures (bad cli.json, missing binary) stay actionable. Ended stdout and
  // broken transport are the same fact from the client's side — the session is
  // gone, not one request.
  ...
  if (isAcpInputStreamEndedError(error) || isAcpTransportError(error)) {
